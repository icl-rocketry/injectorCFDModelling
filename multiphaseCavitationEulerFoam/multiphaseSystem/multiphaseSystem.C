    ...
    )
{
    ...
    interfaceDictTable cavitationModelsDict(lookup("cavitationModel"));
    
    forAllConstIter(interfaceDictTable, cavitationModelsDict, iter)
    {
        pcModels_.insert
        (
            iter.key(),
...
Foam::autoPtr<Foam::multiphaseSystem::cFields>

Foam::multiphaseSystem::cCoeffs() const
{
	autoPtr<cFields> cCoeffsPtr(new cFields);
	forAllConstIter(pcTable, pcModels_, iter)
	{
		const cavitationModel& pc = *iter();
		const Pair<tmp<volScalarField>> vDotAlphal = pc.vDotAlphal();
		volScalarField* Kptr = (vDotAlphal[0]).ptr();
		cCoeffsPtr().insert(iter.key(), Kptr);
	}
	return cCoeffsPtr;
}

Foam::autoPtr<Foam::multiphaseSystem::vFields>

Foam::multiphaseSystem::vCoeffs() const
{
	...
	forAllConstIter(pcTable, pcModels_, iter)
	{
		const cavitationModel& pc = *iter();
		const Pair<tmp<volScalarField>> vDotAlphal = pc.vDotAlphal();
		volScalarField* Kptr = (vDotAlphal[1]).ptr();
		vCoeffsPtr().insert(iter.key(), Kptr);
	}
	return vCoeffsPtr;
}

Foam::autoPtr<Foam::multiphaseSystem::pFields>

Foam::multiphaseSystem::pCoeffs() const
{
	autoPtr<pFields> pCoeffsPtr(new pFields);
	forAllConstIter(pcTable, pcModels_, iter)
	{
		const cavitationModel& pc = *iter();
		const Pair<tmp<volScalarField>> vDotP = pc.vDotP();
		volScalarField* Kptr = (vDotP[1]-vDotP[0]).ptr();
		pCoeffsPtr().insert(iter.key(), Kptr);
	}
	return pCoeffsPtr;
}

Foam::autoPtr<Foam::multiphaseSystem::pSatFields>

Foam::multiphaseSystem::pSats() const
{
	autoPtr<pSatFields> pSatsPtr(new pSatFields);
	forAllConstIter(pcTable, pcModels_, iter)
	{
		const cavitationModel& pc = *iter();
		const Pair<tmp<volScalarField>> vDotP = pc.vDotP();
		const dimensionedScalar psat = pc.pSat();
		volScalarField* Kptr = (psat*(vDotP[1]-vDotP[0])).ptr();
		pSatsPtr().insert(iter.key(), Kptr);
	}
	return pSatsPtr;
}

Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::pcSp
(
	const phaseModel& phase,
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::pcSu
(
	const phaseModel& phase,
	const cFields& cCoeffs,
	const vFields& vCoeffs
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::pcPSp
(
	const pFields& pCoeffs
) const
{
	tmp<volScalarField> tpcPSp
	(
		new volScalarField
		(
			IOobject
	...
	pcTable::const_iterator dmIter = pcModels_.begin();
	pFields::const_iterator dcIter = pCoeffs.begin();
	
	for
	(;
	 dcIter != pcModels_.end() && dcIter != pCoeffs.end();
	 ++dmIter, ++dcIter
	)
	{
		tpcPSp.ref() += *dcIter();
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::pcPSu
(
	const pSatFields& pSats
) const
{
	tmp<volScalarField> tpcPSu
	(
		new volScalarField
		(
			IOobject
	...
	pcTable::const_iterator pmIter = pcModels_.begin();
	pFields::const_iterator pcIter = pSats.begin();
	
	for
	(;
	 pmIter != pcModels_.end() && pcIter != pSats.end();
	 ++pmIter, ++pcIter
	)
	{
		tpcPSu.ref() += *pcIter();
